...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    socket.on('device_status', (data: any) => {
      if (data) setHardwareStatus(prev => ({ ...prev, ...data }));
      fetchStats();
    });
        ...
        <div className="space-y-8">
            ...
            <div className="bg-[#0d121b] border border-white/5 p-8 rounded-[2.5rem]">
                ...
                <div className="space-y-6">
                    {[
                        { 
                          label: 'Procesador ESP32-C6', 
                          status: hardwareStatus.online ? 'Online' : 'Offline', 
                          color: hardwareStatus.online ? 'text-emerald-400' : 'text-rose-400' 
                        },
                        { 
                          label: 'Sensor Biométrico [SFM-V1.7]', 
                          status: hardwareStatus.sensor_conectado ? 'Conectado' : 'Desconectado', 
                          color: hardwareStatus.sensor_conectado ? 'text-emerald-400' : 'text-rose-400' 
                        },
                        { 
                          label: 'Broker MQTT (TLS)', 
                          status: hardwareStatus.online ? 'Enlazado' : 'Inactivo', 
                          color: hardwareStatus.online ? 'text-cyan-400' : 'text-slate-500' 
                        },
                    ].map((item, i) => (
